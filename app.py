...
def callback(event):
    webbrowser.open_new(r"https://github.com/Sameer18-Dev/backpropagation-through-time-in-lstm-visualizer")
...
def playIt():
	...
	#Evals
		...
		plt.xlim(0, epoc)
		ax1 = fig.add_subplot(111, xlim=(0, epoc), ylim=(0, maxMse + (maxMse / 10)))
		ax1.plot(mse, label='Train')  # Use label parameter
		ax1.plot(vmse, label='Val')  # Use label parameter
		fig.legend()  # Call legend once
		fig.suptitle('Evaluation')
		canvas = FigureCanvasTkAgg(fig, master=master)
		canvas.draw()
		canvas.get_tk_widget().grid(row=1, column=6, columnspan=1)
	...
	#Train Graph
		...
		ax2.plot(tt.predict(tt.X_train, tt.Y_train)[0], label='Actual')
		ax2.plot(tt.predict(tt.X_train, tt.Y_train)[1], label='Predicted')
		ax2.legend()
		fig.suptitle('Training Set')
		canvas = FigureCanvasTkAgg(fig, master=master)
		canvas.draw()
	...
	#Test Graph
		...
		ax3.plot(tt.predict(tt.X_test, tt.Y_test)[0], label='Actual')
		ax3.plot(tt.predict(tt.X_test, tt.Y_test)[1], label='Predicted')
		ax3.legend()
		fig.suptitle('Testing Set')
		canvas = FigureCanvasTkAgg(fig, master=master)
		canvas.draw()
